...
%% Process all entries in socket
    ...
    %Se obtienen las matrices de amplitud y fase
    csi_amp_matrix = db(abs(squeeze(csi))); 
    csi_phase_matrix = angle(squeeze(csi));

	%This plot will show graphics about recent 10 csi packets
        set(p(index*3 + 1),'XData', [1:30], 'YData', csi_phase_matrix(1,:), 'color', 'b', 'linestyle', '-');
        if Nrx > 1
            set(p(index*3 + 2),'XData', [1:30], 'YData', csi_phase_matrix(2,:), 'color', 'g', 'linestyle', '-');
        end
        if Nrx > 2
            set(p(index*3 + 3),'XData', [1:30], 'YData', csi_phase_matrix(3,:), 'color', 'r', 'linestyle', '-');
        end
        axis([1,30,-10,10]);
        drawnow;
